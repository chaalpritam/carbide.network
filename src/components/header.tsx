  ...
  return (
    ...
    >
      <nav className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-24 h-20 flex items-center justify-between">
        ...
        >
          <span
            className={`absolute left-2 w-6 h-[2px] bg-black transition-all duration-300 ${
              mobileMenuOpen ? "top-[19px] rotate-45" : "top-3"
            }`}
          />
          <span
            className={`absolute left-2 top-[19px] w-6 h-[2px] bg-black transition-all duration-300 ${
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-xl border-b border-black/5 transition-all duration-500 ${
        ...
        <div className="px-6 py-10 space-y-6">
          {["Technology", "Providers", "Pricing"].map((item) => (
            <Link
              key={item}
              href={`#${item.toLowerCase()}`}
              className="block text-2xl text-black/60 hover:text-black transition-colors"
